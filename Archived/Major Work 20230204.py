...
#Functional test, can move to the 'Help' screen

#-------------------WELCOME----------------------------

welcome = pygame.display.set_mode((520,520))
helpScreen = pygame.display.set_mode((520,520))
pygame.display.set_caption("SDD Assignment 2")
startGameImage = pygame.image.load('night-g824d8643a_1280.jpg')
...
def help():
    ...
    while not game_over and not mainGame and not Home:
        ...
        if (return_button_rect[0] <= x <= return_button_rect[0] + return_button_rect[2] and return_button_rect[1] <= y <=
            ...
            pygame.draw.rect(helpScreen, returnButton.over_colour, return_button_rect)


        welcome.blit(returnButton.text, (return_button_rect[0] + (returnButton.width - returnButton.text.get_width()) / 2,
                                        return_button_rect[1] + (returnButton.height / 2 - returnButton.text.get_height() / 2)))
        welcome.blit(helpName, ((helpScreen.get_width() - helpName.get_width()) / 2, (helpScreen.get_height() - helpName.get_height()) / 2 - 150))
...
def Game():
    ...
    while not game_over and not Help and not Home:
        ...
        menu_button_rect = [(helpScreen.get_width() - menuButton.width) - 20, 20, menuButton.width,
                            menuButton.height]
        menu_help_button_rect = [(welcome.get_width() - menuHelpButton.width) / 2,
                                 (0 - menuHelpButton.height) - 190 + screeny, menuHelpButton.width,
                                 menuHelpButton.height]
        menu_quit_button_rect = [(welcome.get_width() - menuQuitButton.width) / 2,
                                 (0 - menuQuitButton.height) - 70 + screeny, menuQuitButton.width,
                                 menuQuitButton.height]
        menu_home_button_rect = [(helpScreen.get_width() - homeButton.width) / 2,
        ...
        menu_box_outer_rect = [(helpScreen.get_width() - menuBoxOuter.width) / 2, (screeny - menuBoxOuter.height),
                               menuBoxOuter.width, menuBoxOuter.height]
        menu_box_middle_rect = [(helpScreen.get_width() - menuBoxMiddle.width) / 2,
                                (screeny - menuBoxMiddle.height - 5),
                                menuBoxMiddle.width, menuBoxMiddle.height]
        menu_box_inner_rect = [(helpScreen.get_width() - menuBoxInner.width) / 2, (screeny - menuBoxInner.height - 15),
                               menuBoxInner.width, menuBoxInner.height]
        helpScreen.blit(startGameImage, (0, 0))
        if Menu:
            pygame.draw.rect(helpScreen, menuBoxOuter.colour, menu_box_outer_rect)
            pygame.draw.rect(helpScreen, menuBoxMiddle.colour, menu_box_middle_rect)
            pygame.draw.rect(helpScreen, menuBoxInner.colour, menu_box_inner_rect)
            pygame.draw.rect(welcome, homeButton.colour, menu_home_button_rect)
            pygame.draw.rect(welcome, menuQuitButton.colour, menu_quit_button_rect)
            pygame.draw.rect(welcome, menuHelpButton.colour, menu_help_button_rect)
        pygame.draw.rect(helpScreen, menuButton.colour, menu_button_rect)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                game_over = True
        ...
        if (menu_button_rect[0] <= x <= menu_button_rect[0] + menu_button_rect[2] and menu_button_rect[
            1] <= y <= menu_button_rect[1] + menu_button_rect[3]):
            pygame.draw.rect(helpScreen, menuButton.over_colour, menu_button_rect)

        if Menu and (menu_quit_button_rect[0] <= x <= menu_quit_button_rect[0] + menu_quit_button_rect[2] and menu_quit_button_rect[
            1] <= y <= menu_quit_button_rect[1] + menu_quit_button_rect[3]):
            pygame.draw.rect(helpScreen, menuQuitButton.over_colour, menu_quit_button_rect)

        if Menu and (menu_home_button_rect[0] <= x <= menu_home_button_rect[0] + menu_home_button_rect[2] and menu_home_button_rect[
            1] <= y <= menu_home_button_rect[1] + menu_home_button_rect[3]):
            pygame.draw.rect(helpScreen, homeButton.over_colour, menu_home_button_rect)
        if Menu and (menu_help_button_rect[0] <= x <= menu_help_button_rect[0] + menu_help_button_rect[2] and menu_help_button_rect[
            1] <= y <= menu_help_button_rect[1] + menu_help_button_rect[3]):
            pygame.draw.rect(helpScreen, menuHelpButton.over_colour, menu_help_button_rect)

        if Menu:
            welcome.blit(homeButton.text,
            ...
            welcome.blit(menuQuitButton.text, (menu_quit_button_rect[0] + (menuQuitButton.width - menuQuitButton.text.get_width()) / 2,
            ...
            welcome.blit(menuName, ((helpScreen.get_width() - menuName.get_width()) / 2,
                                    0-(helpScreen.get_height() - menuName.get_height()) / 2 - 395))

        helpScreen.blit(menuButton.text,
